...
from document_extractor import process_uploaded_documents, analyze_damage_image, extract_iauditor_images
...
def generate_full_report(case_data):
    ...
    # Reference line - use smaller font and tabs to fit on one line
    ...
    def embed_iauditor_group(doc, category_list, caption_text=None):
        ...
        if caption_text:
            ...
            run.italic = True
            run.font.size = Pt(10)
        # Sort by photo number to preserve report order
        ...
        for img in imgs:
            ...
            if not os.path.exists(img_path):
                continue
            try:
                # Print the iAuditor heading text above the photo (when it changes)
                ...
                # Caption with the original "Photo N" label from iAuditor
                pnum = img.get('photo_number')
                if pnum:
                    cap = doc.add_paragraph()
                    cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    cr = cap.add_run(f"Photo {pnum}")
                    cr.italic = True
                    cr.font.size = Pt(9)
    ...
    # Only embed packaging-specific photos here (not cargo damage — those go in 2.3)
